...
class FolderPredictor:
    ...
    def predict(self):
        self.exit_keywords = ['exit', 'quit', 'q', '']
        ...
        while True:
            ...
            for input_filename in self.load_folder(user_input):
                ...
                for method_prediction_result in method_prediction_results:
                    json_result += "{\n"
                    json_result += "\t\"original_name\": \"" + toJavaMethodName2(
                ...
                # Store the json result in the output folder
                # with the same name as the input file except instead of .java it's .json
                output_filename = os.path.join(OUTPUT_PATH, os.path.basename(input_filename)[:-5] + ".json")
